...
export function fileRecord(messageIdValue: string, attachment: LanAttachment, peerName?: string): LanFileRecord {
	return {
		id: attachment.id,
...
export function isMobileCapability(capability?: LanCapability | null) {
	return capability?.platform === 'android' || capability?.platform === 'ios'
}

export function clampBytes(bytes: number, total: number) {
