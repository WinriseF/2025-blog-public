...
9. `src/lib/lan-transfer/native-webrtc-transport.ts` owns one native `RTCPeerConnection` generation and the single reliable ordered `lan-session-v9` DataChannel. It implements SDP exchange, per-negotiation Candidate buffering, backpressure, transport hello/ready, ping/pong, `restartIce()`, connection-state events, selected-candidate route inspection, and pre-transfer binary frame-size probing. Chunk negotiation probes a 128KB frame with a 124KB payload, then falls back to a 64KB frame with a 60KB payload. `inspectRoute()` reads the selected candidate pair from `getStats()` and exposes only structured family/type labels (`IPv6 直连`, `IPv4 局域网直连`, `IPv4 NAT 直连`, `IPv4 直连`, or `未知直连`); complete addresses are never stored in view state or rendered. STUN (`stun:stun.l.google.com:19302`) is used only for candidate discovery; there is intentionally no TURN or WebSocket file relay fallback.
...
15. Incoming image and voice attachment offers are accepted automatically and cached for in-chat display without opening the browser download flow. Chat images open in a full-screen preview with mouse-wheel and touch-pinch zoom. Automatic media caching never uses File System Access because it requires a user gesture; it uses memory for small media and OPFS/IndexedDB when needed. Ordinary file attachments still wait for the user to click the file card's download action. After that, desktop browsers with File System Access use direct file save first, then progressively falls back to OPFS, IndexedDB, or memory when needed. Direct File and OPFS merge consecutive chunks into about 4MB before each filesystem write; checkpoints, final chunks, and finalization flush any smaller remainder. The sender likewise reads source files in about 4MB batches and slices the negotiated payload chunks from memory. Data frames negotiate between 128KB and 64KB limits; payload chunks are respectively 124KB and 60KB so both tiers reserve 4KB for the frame header. The chosen size is fixed before creating the attachment manifest and remains unchanged during resume. Non-direct file storage creates an object URL for download and keeps the completed cache until the user leaves or clears it from file management.
16. Same-page reconnect is coordinated per device with `connected → suspect → ice-restarting → rebuilding → backoff`. `src/lib/lan-transfer/connection-health-monitor.ts` checks visible connections every eight seconds and probes only after ten seconds without inbound DataChannel traffic. A queue above the 1MB mobile low-water mark suppresses ping/pong so control messages are not timed out behind file frames; falling `bufferedAmount` counts as progress, while both inbound traffic and the queued-byte count must remain stalled for sixty seconds before the queue is considered unhealthy. With a low queue, an idle probe waits five seconds, or ten seconds when some data remains buffered. One missed probe keeps the Transport attached and shows a slow-response status; only a second miss after a three-second retry enters `suspect`. A suspect Transport then receives a four-second natural-recovery grace period, and late inbound traffic can cancel recovery before any detach. ICE Restart has an eight-second timeout, full rebuild has a twelve-second timeout, and repeated failures use capped exponential backoff. Hidden pages do not fail solely because background timers were throttled. Visibility/page restore, browser `online`, and Network Information changes trigger one deduplicated health probe; successful recovery inspects the selected route again. A failed path first attempts ICE Restart and fresh Candidate collection on the existing generation, then performs a full rebuild only if restart fails. Once recovery is required, disconnect aborts and drains the old file sender. A new Transport stays in `resume-syncing`, where the receiver drains its ordered chunk-write queue, flushes the selected storage engine's checkpoint, and returns one authoritative ranges/bytes snapshot. Only after that current response is applied may one new sender loop transmit the missing chunks. Refresh-level or cross-device persistent file resume is not promised.
