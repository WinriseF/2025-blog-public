...
9. `src/lib/lan-transfer/native-webrtc-transport.ts` owns one native `RTCPeerConnection` generation and the single reliable ordered `lan-session-v8` DataChannel. It implements SDP exchange, per-negotiation Candidate buffering, backpressure, transport hello/ready, ping/pong, `restartIce()`, connection-state events, selected-candidate route inspection, and pre-transfer binary frame-size probing. Chunk negotiation tries 512KB, 256KB, and 128KB frames in descending order within the SCTP and peer limits, then falls back to the established 64KB frame size. `inspectRoute()` reads the selected candidate pair from `getStats()` and exposes only structured family/type labels (`IPv6 直连`, `IPv4 局域网直连`, `IPv4 NAT 直连`, `IPv4 直连`, or `未知直连`); complete addresses are never stored in view state or rendered. STUN (`stun:stun.l.google.com:19302`) is used only for candidate discovery; there is intentionally no TURN or WebSocket file relay fallback.
...
11. Chat message list order is the local session insertion order so peer clock skew cannot reorder visible history; sender-created `createdAt` is retained for time display only. Attachment offers reuse the original attachment message time for display. Images and voice messages render as media bubbles once cached, while ordinary files render as file cards that wait for the receiver's download action.
12. V8 DataChannel control messages include `capability`, `chat-message`, `chat-history`, `attachment-offer`, `attachment-accept`, `attachment-progress`, `attachment-complete`, `attachment-received`, `attachment-cancel`, `resume-query`, and `resume-state`. Control messages carry `protocolVersion`, `peerId`, `seq`, and `createdAt` where applicable. Resume messages additionally carry `resumeId`, `transportGeneration`, and the sender runtime's `transportEpoch`; responses that do not match the current connection attempt are ignored. `chat-history` syncs text/system messages and attachment metadata after reconnect; it does not replay file/media bytes or send object URLs. Attachments use `attachmentId + chunkIndex` for ordered validation and idempotent writes. Each binary chunk carries a CRC32 checksum so corrupted chunks are rejected before storage writes.
...
15. Incoming image and voice attachment offers are accepted automatically and cached for in-chat display without opening the browser download flow. Chat images open in a full-screen preview with mouse-wheel and touch-pinch zoom. Automatic media caching never uses File System Access because it requires a user gesture; it uses memory for small media and OPFS/IndexedDB when needed. Ordinary file attachments still wait for the user to click the file card's download action. After that, desktop browsers with File System Access use direct file save first, batching contiguous DataChannel chunks into small streaming disk writes, then progressively fall back to OPFS, IndexedDB, or memory when needed. Data frames negotiate among 512KB, 256KB, 128KB, and 64KB limits; payload chunks are respectively 508KB, 252KB, 124KB, and 60KB so every tier reserves 4KB for the frame header. The chosen size is fixed before creating the attachment manifest and remains unchanged during resume. Non-direct file storage creates an object URL for download and keeps the completed cache until the user leaves or clears it from file management.
16. Same-page reconnect is coordinated per device with `connected → suspect → ice-restarting → rebuilding → backoff`. A transient disconnect receives a four-second natural-recovery grace period, ICE Restart has an eight-second timeout, full rebuild has a twelve-second timeout, and repeated failures use capped exponential backoff. Visible idle connections use transport ping/pong; hidden pages do not fail solely because background timers were throttled. Visibility/page restore, browser `online`, Network Information changes, and successful ICE Restart trigger selected-route inspection again. A failed path first attempts ICE Restart and fresh Candidate collection on the existing generation, then performs a full rebuild only if restart fails. Disconnect aborts and drains the old file sender. A new Transport stays in `resume-syncing`, where the receiver drains its ordered chunk-write queue, flushes the selected storage engine's checkpoint, and returns one authoritative ranges/bytes snapshot. Only after that current response is applied may one new sender loop transmit the missing chunks. Refresh-level or cross-device persistent file resume is not promised.
