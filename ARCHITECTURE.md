...
9. `src/lib/lan-transfer/native-webrtc-transport.ts` owns one native `RTCPeerConnection` generation and the single reliable ordered `lan-session-v8` DataChannel. It implements SDP exchange, per-negotiation Candidate buffering, backpressure, transport hello/ready, ping/pong, `restartIce()`, connection-state events, and selected-candidate route inspection. `inspectRoute()` reads the selected candidate pair from `getStats()` and exposes only structured family/type labels (`IPv6 直连`, `IPv4 局域网直连`, `IPv4 NAT 直连`, `IPv4 直连`, or `未知直连`); complete addresses are never stored in view state or rendered. STUN (`stun:stun.l.google.com:19302`) is used only for candidate discovery; there is intentionally no TURN or WebSocket file relay fallback.
