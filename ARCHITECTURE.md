...
13. Multiple selected files are sent as independent attachments in one message, not as a ZIP. Each attachment has its own progress, completion, failure, and file-record state. Ordinary files remain serial within a file lane and retain the lane until the receiver confirms the completed disk write. Automatically accepted images and voice messages use an independent media lane, and that lane is released as soon as all media frames plus the completion message have been handed to the DataChannel; the later `attachment-received` acknowledgement updates final status but cannot block the next media item at 0%. Both lanes share the V8 unordered data channel and are demultiplexed by attachment id; this does not add a third DataChannel or change the wire shape.
...
16. Same-page reconnect is coordinated per device with `connected → suspect → ice-restarting → rebuilding → backoff`. `disconnected` keeps the current Runtime and Transport attached for a four-second natural-recovery grace period; recovery on the same Transport discards only uncommitted reorder buffers, queries the latest committed snapshot, and resumes missing chunks. Only grace expiry or `failed` detaches the Runtime and starts ICE Restart/rebuild. Active attachments disable application ping death checks; focus, pageshow, visibility restore, online, and Network Information changes refresh signaling/route state without a three-second probe while transfer is active. Resume query retries after 1, 2, and 4 seconds, then clears `resumeSync` and re-sends attachment offers instead of remaining locked. Refresh-level or cross-device persistent resume is not promised.
