						...
						return (
							<div
								...
								<div className='group flex h-10 items-center border-b border-border/45 pr-3 text-sm hover:bg-article/60' style={{ paddingLeft: row.searchResult ? 12 : row.depth * 18 + 10 }}>
									<button type='button' onClick={() => node.kind === 'directory' && toggleExpanded(node.id)} className='flex size-7 shrink-0 items-center justify-center text-secondary' aria-label={isExpanded ? '收起目录' : '展开目录'}>
