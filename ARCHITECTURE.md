...
- `/toolbox/codex-session`: browser-only Codex rollout JSONL audit and multi-Session timeline for key command runs, explicit file reads, successful file patches, request-level Token trends, and project distribution.
...
- `src/app/about/list.json`: about page title and description metadata
- `public/about.md`: about page Markdown content
...
2. Random-character and PIN modes use `crypto.getRandomValues` with rejection sampling. Random-character generation guarantees every enabled character group appears, applies optional exclusions, and can prevent adjacent repeats. Its symbol group contains the 27 printable ASCII punctuation characters left after excluding double quote, single quote, backslash, backtick, and dollar sign to reduce configuration-file quoting, escaping, and interpolation hazards while retaining `#`.
...
`/toolbox/codex-session` imports one or more user-selected Codex `rollout-*.jsonl` files, including a user-selected Session directory, and keeps the source `File` objects only in current-page memory. One file opens the complete Session audit directly. Multiple files produce a local collection timeline with request-level Token trends, project distribution, date/project/model filters, per-file failure isolation, and on-demand drill-down into the existing complete audit. The Token chart expands the imported span into real calendar days, compresses the full visible range into the available plot width instead of hiding later dates behind implicit horizontal scrolling, and keeps its main plot free of selection overlays. A compact navigator below the plot carries the full-span mini bars and two-handle range brush. Pointer movement updates only that local navigator; the selected dates are committed to collection aggregation on pointer release, while keyboard arrows move a focused boundary one day at a time. Every selected rollout is an independent timeline and accounting entry even when multiple files expose the same internal Session ID or identify as a fork/subagent; the collection layer has no duplicate or inherited-Session exclusion. It does not scan `.codex` without a file-picker selection, upload or persist files or summaries, use an API route, connect to the Toolbox Agent, execute commands, or access paths mentioned inside a Session.

The lightweight batch parser and complete detail parser share one turn-performance collector. It associates turn context, user-message time, the first visible assistant message, turn completion, and request-level Output tokens without retaining message bodies. Collection filters also filter these turn records before calculating first-response average/P50/P95, average turn duration, Output per request, and end-to-end Output tokens per second. Saved rollout timestamps are message/event timestamps rather than streaming token deltas, so the UI labels first response as a log-derived approximation and explicitly does not present end-to-end throughput as model-only generation TPS; the latter includes reasoning, tool execution, and waiting inside the completed turn wall-clock interval.

The implementation is split into three boundaries:

- `src/lib/codex-session/` owns the Worker protocol, `stream-chain` Web Stream JSONL ingestion, physical line/byte indexing, permissive Zod validation and record envelopes, Codex record normalization, command-tool and process tracking, `call_id` correlation, restricted Acorn static analysis of outer `exec` JavaScript, Tree-sitter Shell analysis, authoritative command/file audit aggregation, recorded-only Token accounting, compact Session summaries, project-path normalization, and pure collection aggregation. Small protocol helpers stay colocated with the tracker or record utility that consumes them.
- `src/app/toolbox/codex-session/` owns single/multiple/directory file selection and drop, batch/detail progress and cancellation, the collection dashboard, lightweight non-animated HTML Token/project charts, the virtualized Session timeline, on-demand detail parsing, the three fixed command/file/Token detail views, shared view formatting, view-local search and filters, focused command/file/Token details, and the on-demand patch Diff modal. The single-Session request chart remains a separate dynamic Recharts boundary; the detail surface is shared by direct single-file imports and collection drill-down.
...
The main thread sends the selected `File` objects to `parser.worker.ts`. Detail mode streams one file through the JSONL parser, normalizes the Session, and then loads `public/wasm/codex-session/` grammars to analyze Shell batches off the main thread. Batch mode processes files sequentially for predictable peak memory and uses `summary-parser.ts` to extract only shared Session metadata and Token data; it does not construct process output, file evidence, Patch analysis, Exec ASTs, or Shell ASTs. Individual invalid files do not stop the batch. Selecting a timeline item reparses that retained in-memory `File` through detail mode, so month-level state never retains every command output and Patch. Both modes return periodic progress. Process, file, and Token evidence in detail mode retains its source reference, but the product does not expose raw JSONL history. Session strings are rendered through React text nodes or `<pre>` and never through `dangerouslySetInnerHTML`. Replacing or cancelling an import terminates the Worker and advances a request generation so stale results are ignored. Worker initialization failure is surfaced directly and has no whole-file main-thread fallback.
...
Token totals come only from `event_msg.payload.type === "token_count"`. Request samples retain the active turn, cwd, and model from the nearest preceding context record. Collection charts sum recorded `last_token_usage` request increments, bucket them by the browser's local calendar date, and attribute each increment to its active cwd-derived project. Every selected rollout contributes its own final monotonic cumulative total without collection-level deduplication. Any positive difference between that total and its request-sample sum is added as `unallocated` Token at the Session end time and final cwd, so the entire selected-file total remains present in totals and distributions without inventing an input/output breakdown. Input includes cached input, output includes reasoning output, and both subsets are shown without double counting inside one rollout. Missing usage is displayed as unavailable and decreasing cumulative totals are hidden as invalid. There is no tokenizer fallback or cost estimate.

## Architectural Risks And Constraints

## Version Control Workbench

`/toolbox/version-control` is the read-only Git/SVN workbench. It opens public GitHub repositories directly in the browser on any supported browser, and retains the Windows desktop Agent path for local Git/SVN repositories. Its compact launch surface keeps the shared site navigation; after a repository is selected, the focused full-screen workbench hides that navigation. `/toolbox/version-control/agent-return` is the short-lived custom-protocol callback handoff page.

The frontend is split into two boundaries:

- `src/lib/version-control/` owns Version Control Bridge V2, callback validation, the 64KiB control-frame codec, source preview streams, the repository data-source boundary, the Pierre input adapter and parse Worker, graph layout, repository candidates, and the Zustand session store. The workbench reads repositories through `RepositoryDataSource`; the local-Agent adapter binds opaque repository IDs to Bridge calls, while `GitHubRestRepositoryDataSource` implements the same cursor-based history, Diff, and preview contract through browser-direct REST requests.
...
GitHub mode uses anonymous browser-direct requests, fetches 100 default-branch commits per network page, and exposes 30 rows per UI page. The file view loads one recursive Git Trees snapshot for the current branch, builds the directory map in the browser, and falls back to Contents requests for individual directories only when GitHub marks the recursive tree as truncated. GitHub limits a recursive tree response to 100,000 entries or 7 MB; the fallback preserves access to larger repositories at the cost of additional requests. Standalone source previews use `@pierre/diffs/react` `File` inside its `Virtualizer` with the shared Diff themes, while comparisons use `FileDiff`; source cache keys include a content fingerprint so refreshes cannot reuse stale highlighted output. Browser-supported GitHub images load lazily from the Raw content host. Local Git/SVN images up to 16 MiB use a bounded binary preview stream and a short-lived Blob URL that is revoked when the selection changes. Both paths appear in the file pane and reuse the toolbox `ImagePreviewDialog` for full-screen zoom and pan. Commit pages, commit details, comparisons, patches, file contents, and the tree snapshot are deduplicated in RAM and released when the repository is closed or the page reloads. Remote mode has no working tree or export capability, so those controls are omitted.

Local Git history exposes local and remote branch refs on demand. The Agent caches the paged branch snapshot until repository refresh, and rapid checkbox changes are coalesced before history is reloaded. The history filter sends the selected full ref names to the Agent, whose revwalk roots are the selected branch heads; `全部分支` keeps the existing all-ref behavior, while one or more selections show the union of commits reachable from those heads. Branch changes reset history pagination and any commit comparison/Diff state, but do not reload the lazy file tree or working-tree Diff. SVN remains a linear history backend and does not expose this filter.

Local mode launches the existing portable EXE with `winrisef://launch?...&feature=version-control`. This mode is mutually exclusive with `/t` native acceleration and accepts only a pinned loopback WebTransport endpoint at `/winrisef/version-control/v2`. Every Agent process grants repositories through the native directory picker. Git is read through vendored libgit2; SVN is read through the controlled system `svn.exe` command line with bounded output, `--non-interactive`, `--no-auth-cache`, no shell, and the authorized working-copy root as the child process current directory. Local SVN targets are relative to that root; URL/peg targets are kept separate from local paths. SVN working-copy differences use BASE-to-WORKING semantics and derive the normal local file list from `svn status`; historical comparisons keep `svn diff --summarize` separate from mixed-revision working-copy state. If a selected directory contains both Git and SVN metadata, the Agent returns opaque candidates and the browser requires an explicit choice. The browser never submits an arbitrary local path and does not persist recent repositories. Opening a non-bare local repository reads only its overview and first history page; the working-tree Diff is opened explicitly, and the local file tree loads one directory level per expansion. Directory metadata and working-tree source inspection skip inaccessible child entries so one permission failure does not abort the whole response.
...
If you need to update the about page, edit `public/about.md` for the rendered content and `src/app/about/list.json` for its title or description.
