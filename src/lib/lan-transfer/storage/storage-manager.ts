...
import { DirectFileStorageEngine } from './direct-file-storage'
...
export function chooseStorageKind(size: number, requested: LanStorageKind, capability: LanCapability | null): LanStorageKind {
	...
	if (requested === 'file' && capability?.storage.fileSystemAccess) return 'file'
	...
	if (capability?.storage.fileSystemAccess) return 'file'
