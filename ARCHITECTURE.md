...
3. LAN Session V6 is a full-viewport chat-style workbench instead of a panel inside the toolbox shell. Entering LAN mode renders an independent QQ-like app surface that covers the original toolbox layout and route chrome. Desktop renders a two-column layout: a device/session sidebar with the live connection list on the left, and the selected connection's full chat window on the right. Mobile renders two layers: a device page first, then a pure chat page with only the fixed chat header, independently scrolling message stream, and fixed composer. The message stream follows QQ-style grouping with centered time pills, incoming sender labels above the bubble, constrained text/image/file bubble widths, and no separate per-message timestamp row. There is no standalone file panel or mobile file tab; files stay as chat attachments.
4. Any device can create a pairing QR code as WebRTC `host`; any number of other devices can scan `/t#mode=lan&room=<roomId>&token=<roomToken>` and join that room as separate `guest` peers. The host keeps the same QR/link available so more devices can join later. The browser removes the raw token from the address bar after reading it and stores only the current V6 invite in sessionStorage.
5. The QR token stays browser-side. The browser hashes it locally and sends only `tokenHash` in Supabase Realtime payloads.
6. Both browsers subscribe to the public Realtime channel `lan-transfer:<roomId>`, track peer presence, and use Broadcast only for the `lan` signaling event. Presence payloads carry the current volatile `peerId`, `role`, `peer`, `tokenHash`, and `joinedAt`; Broadcast payloads carry `roomId`, `tokenHash`, `from`, `to`, `seq`, and `ts`. The local device identity is generated once and stored in localStorage: a stable `deviceId` for session merging, a stable English friendly name such as `Sunny Phone A7K2`, plus a random `avatarSeed` used to render a round DiceBear `bottts-neutral` avatar. The avatar CDN receives only that random seed, not device platform strings such as `Linux armv81`.
7. Supported signaling messages are `announce`, `signal`, and `peer-left`. Signaling is routed by volatile `peer.id`, while the UI conversation, chat state, and transfer runtime are keyed by stable `deviceId`. If the same physical browser rejoins the same room with a new `peer.id`, the old WebRTC instance is replaced and the existing conversation is reused. The host creates one `simple-peer` instance per guest device, guests connect only to the host and ignore other guests. No database tables, Supabase Storage objects, service role key, or secret key are used.
8. `announce` is a recovery heartbeat while the LAN page is open: it runs quickly for the first recovery window, then drops to a lower frequency. The client also re-tracks presence and immediately announces on Supabase resubscribe, page focus, visibility restore, `online`, and `pageshow`, so mobile screen-off/screen-on recovery does not depend on manually rescanning. Re-scanning the same invite creates a new volatile peer id but keeps the same device conversation.
9. The browser uses `simple-peer` with STUN-assisted WebRTC config (`stun:stun.l.google.com:19302`) to exchange offer/answer/ICE through Supabase Broadcast, then opens a WebRTC DataChannel named `lan-session-v6`. STUN is used only for candidate discovery; there is intentionally no TURN server or WebSocket file relay fallback for this hotspot transfer mode.
10. Once connected, each host/guest pair is an equal WebRTC data connection. Either side can send text messages, recorded voice messages, images, or files to the currently selected peer. The high-cohesion transfer core is `src/lib/lan-transfer/connection-runtime.ts`: one runtime instance owns one device connection's protocol dispatch, file list, queues, chunk validation, progress, cancellation, same-page resume state, and lightweight chat history sync. `src/app/toolbox/use-lan-transfer-engine.ts` is the multi-connection adapter that owns the device-keyed connection table, selected peer, per-device chat state, and one runtime per device. React UI, SimplePeer, Supabase signaling, QR code, and voice recording are adapters around that runtime.
...
12. V6 control messages include `capability`, `chat-message`, `chat-history`, `attachment-offer`, `attachment-accept`, `attachment-progress`, `attachment-complete`, `attachment-received`, `attachment-cancel`, `resume-query`, and `resume-state`. Control messages carry `protocolVersion`, `peerId`, `seq`, and `createdAt` where applicable. `chat-history` syncs text/system messages and attachment metadata after reconnect; it does not replay file/media bytes or send object URLs. Attachments use `attachmentId + chunkIndex` for ordered validation and idempotent writes. Each binary chunk carries a CRC32 checksum in the chunk header so corrupted chunks are rejected before storage writes.
...
16. Same-page reconnect is supported: outgoing attachment queues and incoming received ranges are retained in memory, and after WebRTC reconnect the peers exchange `resume-query` / `resume-state` so the sender can skip chunks already acknowledged by the receiver. Progress acknowledgements are throttled by byte/time windows to avoid control-message flooding during large transfers. Chat progress bars use receiver-confirmed written bytes, not local DataChannel buffered bytes; speed and ETA are computed from those confirmed bytes so progress does not jump backward. If the receiver completed a file but the final confirmation was lost, reconnecting re-sends the completion acknowledgement instead of asking the user to download the same file again. Refresh-level or cross-device persistent file resume is not promised.
