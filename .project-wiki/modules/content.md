...
| `src/lib/news.ts`, `src/app/api/` | News API normalization. |
| `supabase/functions/like/`, `supabase/migrations/` | Like endpoint and data model. |

## Main Flow

The index fetches `index.json`; an article fetches its paired JSON/Markdown files. Markdown normally renders in a Worker, then becomes TOC, Shiki code blocks, Mermaid placeholders, safe links, and shared image preview. News handlers normalize remote sources. Likes call the external Supabase function.
