...
- `src/lib/version-control/` owns Version Control Bridge V2, callback validation, the 64KiB control-frame codec, source preview streams, graph layout, repository candidates, and the Zustand session store.
- `src/app/toolbox/version-control/` owns launch/setup UI, explicit Git/SVN candidate selection, the virtualized single-lane-compatible commit graph, working-tree groups, file tree, Monaco diff modal, conflict perspectives, and export confirmation.
...
Control commands and paged metadata remain below 64KiB. Preview bodies use independent incoming WebTransport streams. SVN defaults to a lightweight Patch reviewer with real old/new line numbers and explicit omitted-line rows; its `仅变更` mode reads the already-generated Patch and does not launch per-file commands. `完整文件` loads the two UTF-8 source sides into Monaco with a 2MiB limit per side. Binary, invalid UTF-8, and oversized full-file previews remain metadata-only. Rapid history selections are debounced and serialized with a latest-generation guard, so stale comparisons cannot replace the current selection or request pages from an expired Diff session; history search and pagination use the same stale-response guard and merge identical in-flight pages. The file tree is built with one path map and stores each directory's selectable file IDs instead of rescanning the full Diff on every render. Its A/M/D/R status counters are multi-select filters; conflicts add a C filter only when present, and hiding a status also removes those hidden files from the export selection. Monaco models are detached before disposal; full previews use a 30-second bounded Diff computation, automatically wrap long lines, hide horizontal scrollbars, and keep the overview ruler disabled. The Diff modal closes through its close control, Escape, or a direct click on the surrounding backdrop without treating clicks inside the editor as dismissals.
