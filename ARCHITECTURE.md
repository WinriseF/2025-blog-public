...
13. Multiple selected files are sent as independent attachments in one message, not as a ZIP. Each attachment has its own progress, completion, failure, and file-record state. Ordinary files remain serial within a file lane, while automatically accepted images and voice messages use an independent media lane, so a large file that is sending or waiting for disk confirmation cannot leave later chat media stuck at 0%. Both lanes share the V8 unordered data channel and are demultiplexed by attachment id; this does not add a third DataChannel or change the wire shape.
