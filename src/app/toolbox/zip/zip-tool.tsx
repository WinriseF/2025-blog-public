...
type Phase = 'idle' | 'scanning' | 'ready' | 'running' | 'done' | 'canceled' | 'error'
...
export function ZipTool() {
	...
	const selectionStates = useMemo(() => scan ? deriveZipSelection(scan.nodes, selected) : new Map(), [scan, selected])
	const selectedEntryIds = useMemo(() => scan ? selectedZipEntries(scan.nodes, selectionStates) : new Set<string>(), [scan, selectionStates])
	const stats = useMemo(() => scan ? zipSelectionStats(scan.nodes, selected) : { files: 0, bytes: 0 }, [scan, selected])

	const installScan = (nextScan: ZipScanResult, keepRoot: boolean) => {
		scanVersion.current += 1
		setScan(nextScan)
		setSelected(initialZipSelection(nextScan.nodes))
	...
	const chooseDirectory = async () => {
		...
		try {
			...
			setError(null)
			setPhase('scanning')
			const nextScan = await scanZipDirectory(directory, controller.signal, (files, bytes) => setScanProgress({ files, bytes }))
	...
	const startWriting = async () => {
		if (!scan || !selectedEntryIds.size) return
		...
		const controller = new AbortController()
		activeController.current = controller
		setWriteProgress({ ...EMPTY_PROGRESS, totalFiles: stats.files, totalBytes: stats.bytes })
		setError(null)
		setPhase('running')
		try {
			const output = await writeZipArchive({ scan, selectedIds: selectedEntryIds, compression, includeRoot, outputHandle, signal: controller.signal, onProgress: setWriteProgress })
		...
		<div className='mx-auto max-w-6xl text-sm max-sm:px-4'>
			...
			{phase === 'scanning' && (
				<section className='flex min-h-80 flex-col items-center justify-center rounded-2xl border border-border bg-background/20 p-8 text-center'>
					...
					<h2 className='mt-5 font-semibold text-primary'>正在分析目录</h2>
					<p className='text-secondary mt-2'>{scanProgress.files.toLocaleString('zh-CN')} 个文件 · {formatBytes(scanProgress.bytes)}</p>
					<button type='button' onClick={cancel} className='mt-6 rounded-xl border border-border px-4 py-2.5 text-secondary transition hover:text-primary'>取消读取</button>
				</section>
			)}
				...
				<div className='grid min-h-0 gap-5 lg:grid-cols-[minmax(0,1fr)_340px]'>
					<div className={mobileView === 'files' ? '' : 'max-lg:hidden'}>
						<ZipFileTree
							...
							onToggle={(id, checked) => setSelected(current => toggleZipSubtree(nodesById, current, id, checked))}
							onSelectAll={() => setSelected(new Set(scan.nodes.map(node => node.id)))}
							onRestoreSuggestions={() => setSelected(initialZipSelection(scan.nodes))}
					...
					<aside className={`space-y-4 ${mobileView === 'plan' ? '' : 'max-lg:hidden'}`}>
						...
						<section className='rounded-2xl border border-border bg-background/20 p-5'>
							<div className='grid grid-cols-2 gap-3'>
								...
								<Stat label='已排除' value={formatBytes(Math.max(0, scan.totalBytes - stats.bytes))} />
								<Stat label='格式' value={stats.bytes >= 4 * 1024 ** 3 || stats.files > 65535 ? 'ZIP64' : 'ZIP'} />
							</div>
							<button type='button' disabled={!selectedEntryIds.size || !supported} onClick={() => void startWriting()} className='bg-brand mt-5 flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3.5 font-semibold text-background transition hover:opacity-90 disabled:cursor-not-allowed disabled:bg-border disabled:text-secondary'>
			...
			<div className='mt-7 flex items-start gap-3 border-t border-border pt-5 text-xs leading-5 text-secondary'>
				...
				<p>首次打包会从 CDN 加载压缩内核；文件仅在当前设备中读取和打包，不上传服务器。处理期间请保持页面打开，关闭页面会终止任务。</p>
			</div>
		</div>
	)
}
