...
3. LAN Session V4 is a full-viewport chat-style workbench instead of a panel inside the toolbox shell. Entering LAN mode renders an independent QQ-like app surface that covers the original toolbox layout and route chrome. Desktop renders a two-column layout: device/session sidebar on the left and the full chat window on the right. Mobile renders two layers: a device page first, then a pure chat page with only the fixed chat header, independently scrolling message stream, and fixed composer. There is no standalone file panel or mobile file tab; files stay as chat attachments.
