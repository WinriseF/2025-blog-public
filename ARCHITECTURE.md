...
9. Once connected, both devices are equal peers. Either side can send text messages, recorded voice messages, images, or files. Text is sent as a control frame; voice/image/file attachments use a shared chunk protocol. Chat messages are displayed by sender-created `createdAt` time with a deterministic peer/id tie-breaker, and attachment offers reuse the original attachment message time so both devices sort the same conversation consistently.
