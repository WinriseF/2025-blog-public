...
- `src/lib/codex-session/` owns the Worker protocol, `stream-chain` Web Stream JSONL ingestion, physical line/byte indexing, permissive Zod validation and record envelopes, Codex record normalization, command-tool and process tracking, `call_id` correlation, restricted Acorn static analysis of outer `exec` JavaScript, Tree-sitter Shell analysis, authoritative command/file audit aggregation, and recorded-only Token accounting. Small protocol helpers stay colocated with the tracker or record utility that consumes them.
- `src/app/toolbox/codex-session/` owns file selection/drop, parse progress and cancellation, the Session import/result surface, the three fixed command/file/Token views, shared view formatting, view-local search and filters, TanStack-virtualized lists, the dynamically loaded Recharts graph, focused command/file/Token details, and the on-demand patch Diff modal. Worker and Recharts entrypoints remain separate bundle boundaries; small UI-only types and labels stay with their owning component or formatter.
