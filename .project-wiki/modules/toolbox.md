...
- Preserve the distinction between optional compression and mandatory user intent. Original-file fallback is allowed only for keep/auto mode when dimensions and requested metadata behavior are unchanged. Explicit conversion, resize, or metadata removal may legitimately produce a larger file and must say so.
- Treat orientation and color as pixel semantics, not disposable metadata. Browser decode applies orientation once. Auto/keep mode preserves unmodified wide-gamut images unless the user explicitly requests conversion, resizing, or metadata removal.
- libimagequant is GPL-3.0-or-later/commercial despite its wrapper being MIT. Keep the user-visible disclosure and `THIRD_PARTY_NOTICES.md`; do not ship a production deployment under an incompatible licensing assumption.
