...
6. Both browsers subscribe to the public Realtime channel `lan-transfer:<roomId>`, track peer presence, and use Broadcast only for the `lan` signaling event. Presence payloads carry `peerId`, `role`, `peer`, `tokenHash`, and `joinedAt`; Broadcast payloads carry `roomId`, `tokenHash`, `from`, `to`, `seq`, and `ts`. The local device display identity is generated once and stored in localStorage: a stable English friendly name such as `Sunny Phone A7K2`, plus a random `avatarSeed` used to render a round DiceBear `bottts-neutral` avatar. The avatar CDN receives only that random seed, not device platform strings such as `Linux armv81`.
