...
3. LAN Session V8 is a full-viewport chat-style workbench instead of a panel inside the toolbox shell. Entering LAN mode renders an independent QQ-like app surface that covers the original toolbox layout and route chrome. Desktop renders a two-column layout: a device/session sidebar with the live connection list on the left, and the selected connection's full chat window on the right. Mobile renders two layers: a device page first, then a pure chat page with only the fixed chat header, independently scrolling message stream, and fixed composer. The message stream follows QQ-style grouping with centered time pills, incoming sender labels above the bubble, constrained text/image/file bubble widths, and no separate per-message timestamp row. There is no standalone file panel or mobile file tab; files stay as chat attachments.
4. Any device can create a pairing QR code as WebRTC `host`; any number of other devices can scan `/t#mode=lan&room=<roomId>&token=<roomToken>` and join that room as separate `guest` peers. The host keeps the same QR/link available so more devices can join later. WeChat and QQ embedded browsers show a copyable invite-link dialog instead of starting the LAN session; normal browsers remove the raw token from the address bar after reading it and store only the current V8 invite in sessionStorage.
5. The QR token stays browser-side. The browser hashes it locally and sends only `tokenHash` in Supabase Realtime payloads.
6. Both browsers subscribe to the public Realtime channel `lan-transfer:<roomId>`, track peer presence, and use Broadcast only for the `lan` signaling event. A stable localStorage `deviceId` keys the long-lived conversation/runtime, while a volatile `instanceId` identifies the current page instance. V8 signaling also carries `generation`, `negotiationId`, `messageId`, and `seq`, so stale connection generations and stale negotiation candidates are discarded instead of contaminating a rebuilt connection. The stable English friendly name and random DiceBear `avatarSeed` remain device-local metadata.
7. Supported V8 signaling messages are `announce`, `reconnect-request`, `rebuild`, `ice-restart`, `offer`, `answer`, `candidate`, `signal-ack`, and `peer-left`. The host owns each guest connection generation and is the only side that creates offers; guests request recovery and answer host offers. Rebuild/restart/offer/answer messages use application-level ACK, bounded resend, and message-id deduplication. Candidate delivery is duplicate-tolerant and cached by `generation + negotiationId` until the matching remote description is installed. IPv4 and IPv6 Candidates are passed through unchanged; the application does not filter an address family or rewrite SDP, leaving path selection to browser ICE.
8. `src/lib/lan-transfer/signal-client.ts` tracks Realtime independently as `connecting`, `online`, `retrying`, `offline`, or `closed`. Every resubscribe re-tracks Presence, announces the local instance, and flushes pending critical signals. Page focus, visibility restore, `online`, and `pageshow` trigger an immediate recovery pass. If a visible page remains unsubscribed for about 15 seconds, the stale Channel is removed and recreated instead of relying indefinitely on its first ready promise.
9. `src/lib/lan-transfer/native-webrtc-transport.ts` owns one native `RTCPeerConnection` generation and two reliable DataChannels. `lan-control-v8` is ordered and carries chat/control, progress, flow control, resume, and ping/pong; `lan-data-v8` is unordered and carries only file chunks plus throughput probes. Transport ready requires both channels to be open and the control hello to complete. Backpressure reads only the data channel. Before an attachment is created, the transport sends about 1MB of sustained 128KB probe frames and waits for an acknowledgement over the control channel; success selects a 124KB payload, otherwise the attachment uses the 60KB fallback. `inspectRoute()` exposes only structured family/type labels (`IPv6 直连`, `IPv4 局域网直连`, `IPv4 NAT 直连`, `IPv4 直连`, or `未知直连`); complete addresses are never stored or rendered. STUN (`stun:stun.l.google.com:19302`) is used only for candidate discovery; there is intentionally no TURN or WebSocket file relay fallback.
10. Once connected, each host/guest pair is an equal WebRTC data connection. Either side can send text messages, recorded voice messages, images, or files to the currently selected peer. `src/lib/lan-transfer/reconnect-coordinator.ts` owns the per-device connection state machine and replaces only the temporary Transport. `src/lib/lan-transfer/connection-runtime.ts` dispatches chat/control and composes `attachment-sender.ts` with `attachment-receiver.ts`; `incoming-attachment-writer.ts` owns per-attachment bounded reordering and sequential disk commits. `src/app/toolbox/use-lan-transfer-engine.ts` keeps one Runtime per stable device and rejects control/data or async completion callbacks from an obsolete Transport id/epoch.
...
12. V8 control messages include `capability`, `chat-message`, `chat-receipt`, `chat-history`, `attachment-offer`, `attachment-accept`, `attachment-progress`, `attachment-complete`, `attachment-received`, `attachment-cancel`, `resume-query`, and `resume-state`. Progress and resume snapshots report committed bytes/ranges, receive-window bytes, queued bytes, storage, and any pause reason. Resume messages also carry `resumeId`, `transportGeneration`, and `transportEpoch`; stale responses are ignored. Data frames contain only attachment id, chunk index, and payload. There is no application CRC32 or compatibility branch; integrity relies on DTLS/SCTP plus final file byte/chunk-count validation. V8 stayed numerically unchanged, so both peers must refresh and re-pair after deployment.
13. Multiple selected files are sent as independent attachments in one message, not as a ZIP. Each attachment has its own progress, completion, failure, and file-record state. Ordinary files remain serial within a file slot and retain it until the receiver confirms the completed disk write; automatically accepted images and voice messages use a separate media slot that is released after its frames and completion message are handed to the DataChannel. A single sender scheduler alternates one chunk at a time between the eligible file and media slots, skips a slot whose receive window is full, and owns the only shared DataChannel backpressure waiter. The completion message is sent immediately after an attachment's last frame instead of waiting for the whole shared channel to drain; the receiver already waits for all unordered chunks before finalizing. This keeps file/media concurrency without adding a third DataChannel or changing the V8 wire shape.
...
15. Incoming image and voice offers are accepted automatically and cached for in-chat display; ordinary files wait for a user download gesture. Direct File remains the preferred desktop large-file path, followed by OPFS, IndexedDB, or memory. Unordered data chunks are held in a bounded per-attachment Map and only contiguous indexes are committed. Direct File combines contiguous data into writes of up to about 4MB, and the manifest advances only after the write resolves. The receive window targets about two seconds of measured disk throughput, clamped to 2–8MB on mobile and 2–16MB on desktop. A full or 15-second-stalled writer advertises window zero; 60 seconds without a commit aborts only that attachment and keeps chat online. The only data tiers are 128KB frame/124KB payload and 64KB frame/60KB payload, fixed for the attachment.
16. Same-page reconnect is coordinated per device with `connected → suspect → ice-restarting → rebuilding → backoff`. `disconnected` keeps the current Runtime and Transport attached for a four-second natural-recovery grace period; recovery on the same Transport discards only uncommitted reorder buffers, queries the latest committed snapshot, and resumes missing chunks. Only grace expiry or `failed` detaches the Runtime and starts ICE Restart/rebuild. Only an executing sender scheduler or active receiver writer disables application ping death checks; attachments waiting for acceptance or final receipt do not keep a dead connection marked active. Focus, pageshow, visibility restore, online, and Network Information changes refresh signaling/route state without a three-second probe while transfer is active. Resume query retries after 1, 2, and 4 seconds, then clears `resumeSync` and re-sends attachment offers instead of remaining locked. Refresh-level or cross-device persistent resume is not promised.
...
19. `LanTransferDiagnostics` is sampled into React at most once per second and is available from the compact chat-header menu immediately left of the close button. The desktop trigger shows separate upload and download rates, with disk commit rate and the 60/124KB tier added on wider screens; mobile keeps only the trigger icon. The expanded panel is rendered as a fixed `document.body` portal so the chat header, message scroller, and full-screen overflow rules cannot clip or cover it. It shows structured route/state, three separate live rate cards, data-channel buffered bytes, receive queue/window, local sender watermarks and maximum uncommitted bytes, and recent suspect/ICE Restart/rebuild/recovery reasons. Full IP addresses are never shown.
