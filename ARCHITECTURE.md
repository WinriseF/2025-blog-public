...
13. Multiple selected files are sent as independent attachments in one message, not as a ZIP. Each attachment has its own progress, completion, failure, and file-record state.
...
19. `LanTransferDiagnostics` is sampled into React at most once per second and is available from the compact chat-header menu immediately left of the close button. It shows structured route/state, 60/124KB tier, network send/receive speed, disk commit speed, data-channel buffered bytes, receive queue/window, local sender watermarks and maximum uncommitted bytes, and recent suspect/ICE Restart/rebuild/recovery reasons. Mobile renders only the trigger icon; full IP addresses are never shown.
