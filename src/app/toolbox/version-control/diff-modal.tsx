...
export function DiffModal() {
	...
	useEffect(() => {
		...
		setPreview(null)
		setError(null)
		if (!activeFile || !bridge || !repositoryId || !diff || activeFile.isBinary || activeFile.previewTooLarge) return
		const key = previewKey(diff.diffId, activeFile.fileId, perspective)
		const timer = window.setTimeout(() => {
			void bridge
				.openPreview(repositoryId, diff.diffId, activeFile.fileId, perspective)
				.then(value => {
	...
	}, [activeFile, bridge, diff, perspective, repositoryId])
	...
	const modelKey = previewKey(diff?.diffId || 'none', activeFile.fileId, perspective)
	const currentPreview = preview?.key === modelKey ? preview.content : null
	const unavailable = activeFile.isBinary
		? '二进制文件仅展示元数据，不加载正文。'
		: activeFile.previewTooLarge
			? '文件超过单侧 2MiB 预览上限，可在未超过导出上限时完整导出。'
			: null
	const copy = async () => {
		if (!currentPreview) return
		await navigator.clipboard.writeText(`--- a/${activeFile.oldPath || activeFile.path}\n+++ b/${activeFile.path}\n${currentPreview.modified}`)
		setCopied(true)
		...
		<div className='fixed inset-0 z-[140] flex items-center justify-center bg-black/75 p-5 backdrop-blur-sm'>
			<section className='border-border bg-background flex h-[min(900px,94dvh)] w-[min(1500px,96vw)] flex-col overflow-hidden rounded-xl border shadow-2xl'>
				...
				<footer className='border-border bg-article text-secondary flex h-8 items-center border-t px-4 font-mono text-[10px]'>
					<span>READ ONLY</span>
					<span className='ml-auto'>Esc 关闭 · {sideBySide ? 'SIDE BY SIDE' : 'UNIFIED'}</span>
				</footer>
			</section>
		</div>
	)
...
function previewKey(diffId: string, fileId: number, perspective: ConflictPerspective) {
	return `${diffId}:${fileId}:${perspective}`
}
